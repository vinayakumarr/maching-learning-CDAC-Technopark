...
if not os.path.isfile(OUTPUT_FILE):
    ...
    tok = Tokenizer(char_level=True, lower=True, oov_token='UNK')
    ...
    maxlen = np.max([len(x) for x in X11])
    print(maxlen)
    maxlen = np.max([len(x) for x in X21])
    print(maxlen)
    maxlen = np.max([len(x) for x in Xv11])
    print(maxlen)
    maxlen = np.max([len(x) for x in Xv21])
    print(maxlen)
    maxlen = np.max([len(x) for x in Xt11])
    print(maxlen)
    maxlen = np.max([len(x) for x in Xt21])
    print(maxlen)
