...
for file in os.listdir("logs/cnn/"):
  model.load_weights("logs/cnn/"+file)
  y_pred = model.predict_classes(X_test)
...
# try using different optimizers and different optimizer configs
model.load_weights("logs/cnn/checkpoint-15.hdf5")

y_pred = model.predict_classes(X_test)
np.savetxt("cs-res/cnn1.txt",y_pred, fmt="%01d")
...
y_pred = model.predict_classes(X_test1)
np.savetxt("cs-res/cnn2.txt",y_pred, fmt="%01d")
