...
for file in os.listdir("logs/cnnlstm/"):
  model.load_weights("logs/cnnlstm/"+file)
  y_pred = model.predict_classes(X_test)
  accuracy = accuracy_score(y_testn, y_pred)
  recall = recall_score(y_testn, y_pred , average="weighted")
  precision = precision_score(y_testn, y_pred , average="weighted")
...
# try using different optimizers and different optimizer configs
model.load_weights("logs/cnnlstm/checkpoint-04.hdf5")

y_pred = model.predict_classes(X_test)
np.savetxt("cs-res/cnnlstm1.txt",y_pred, fmt="%01d")

model.load_weights("logs/cnnlstm/checkpoint-08.hdf5")

y_pred = model.predict_classes(X_test1)
np.savetxt("cs-res/cnnlstm2.txt",y_pred, fmt="%01d")
